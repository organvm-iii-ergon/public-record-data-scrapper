...
```css
--font-sans:
  'IBM Plex Sans', ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto,
...
.priority-score {
  ...
  font-variant-numeric: tabular-nums;
  color: oklch(0.65 0.3 45);
  text-shadow: 0 0 20px oklch(0.65 0.3 45 / 0.3);
}
```
...
```css
.gradient-text {
  background: linear-gradient(135deg, oklch(0.65 0.3 45), oklch(0.7 0.15 60));
...
.data-display {
  font-feature-settings:
    'tnum' 1,
    /* Tabular numerals */ 'case' 1,
    /* Case-sensitive forms */ 'zero' 1; /* Slashed zero */
}
```
