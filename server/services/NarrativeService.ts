...
export class NarrativeService {
  ...
  private generateSummary(data: NarrativeProspectData): string {
    ...
    if (data.defaultDate && data.daysSinceDefault !== undefined && data.daysSinceDefault > 0) {
      ...
      if (years >= 1) {
        parts.push(
          `${data.companyName} defaulted ${years} year${years > 1 ? 's' : ''} ago but remains ${data.status}`
        )
      } else {
    ...
    if (data.growthSignals.length > 0) {
      parts.push(
        `with ${data.growthSignals.length} growth signal${data.growthSignals.length > 1 ? 's' : ''}`
      )
    }
    ...
    if (data.estimatedStackPosition <= 1) {
      ...
      parts.push(
        `- potential ${data.estimatedStackPosition === 2 ? '2nd' : '3rd+'} position opportunity`
      )
    }
  ...
  private generateGrowthParagraph(data: NarrativeProspectData): string {
    ...
    if (hasHiring && hasExpansion) {
      paragraph +=
        ' The combination of hiring activity and expansion signals suggests active business growth.'
    } else if (hasPermit) {
  ...
  private detectWhaleOpportunity(data: NarrativeProspectData): {
    ...
    if (
      this.whaleConfig.minEstimatedRevenue &&
    ...
    return isWhale ? { isWhale: true, score, reasons } : { isWhale: false }
  }
  ...
  private analyzeGrowth(data: NarrativeProspectData): {
    ...
    if (topSignals.length === 0) {
      return {
        analysis:
          'No recent growth signals detected. May indicate stable but not expanding business.',
        topSignals: []
      }
    }
    ...
    if (signalTypes.includes('hiring') && signalTypes.includes('expansion')) {
      analysis =
        'Strong growth trajectory with both hiring and expansion activity. Business is actively investing in growth - ideal time for financing.'
    } else if (
      signalTypes.includes('permit') &&
      (signalTypes.includes('equipment') || signalTypes.includes('expansion'))
    ) {
      analysis =
        'Physical expansion underway based on permit activity. May have immediate capital needs for build-out or equipment.'
    } else if (signalTypes.includes('hiring')) {
      analysis =
        'Hiring activity suggests growing operations and potentially increased revenue. Working capital may be needed for payroll expansion.'
    } else if (signalTypes.includes('contract')) {
      analysis =
        'New contract wins indicate growing demand. May need financing to fulfill larger orders or projects.'
    } else {
  ...
  private generateStackInsight(data: NarrativeProspectData): {
    ...
    if (data.activeFilings === 0) {
      insight =
        'Clean slate - no current positions. This is a 1st position opportunity with maximum security.'
  ...
  private determineApproach(
    ...
    if (isWhale && riskLevel === 'low' && data.compositeScore >= 70) {
      return {
        ...
        reasoning:
          'High-value opportunity with strong fundamentals. Prioritize this prospect for immediate outreach.'
      }
    }
    ...
    if (data.compositeScore >= 55 && riskLevel !== 'high') {
      return {
        ...
        reasoning:
          'Solid prospect with acceptable risk profile. Standard due diligence and competitive offer recommended.'
      }
    }
    ...
    if (data.compositeScore >= 40 || (riskLevel === 'medium' && data.compositeScore >= 50)) {
      return {
        ...
        reasoning:
          'Proceed with enhanced due diligence. Consider conservative terms or additional requirements.'
      }
    }
    ...
    return {
      recommendation: 'pass',
      reasoning:
        'Risk factors outweigh potential opportunity. Recommend waiting for improved indicators or declining.'
    }
  }
  ...
  private generateObjectionHandlers(data: NarrativeProspectData): ObjectionHandler[] {
    ...
    if (data.activeFilings > 0) {
      handlers.push({
        ...
        response:
          "That's actually common among the businesses we work with. Many use a second or third position to fund specific growth initiatives without disturbing their existing arrangements. What projects are you considering?",
    ...
    handlers.push({
      ...
      response:
        'I hear that concern often. The key is looking at the cost relative to the opportunity. If this funding helps you capture a contract or expand faster, the ROI can far exceed the cost. What opportunity are you trying to capture?'
    })

    // "We don't need money right now"
    if (data.growthSignals.length > 0) {
      const signalPhrase =
        data.growthSignals[0].type === 'hiring'
          ? 'hiring'
          : data.growthSignals[0].type === 'expansion'
            ? 'expanding'
            : data.growthSignals[0].type === 'permit'
              ? 'working on permits'
              : 'growing'
      handlers.push({
    ...
    handlers.push({
      ...
      response:
        'Of course. Would it be helpful if I sent you a quick summary of what we discussed and potential terms? That way you have something concrete to review. When would be a good time to follow up?'
    })
