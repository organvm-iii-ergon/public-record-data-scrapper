...
const compatibility = JSON.parse(execFileSync('python3', ['-c',
  ...
  fileURLToPath(new URL('wrangler.toml', edge))], {encoding:'utf8'}))
...
const compiled = await build({stdin: {contents: `
  ...
  }};`, resolveDir: fileURLToPath(edge)}, bundle:true, format:'esm', platform:'browser', write:false})
const worker = new Miniflare(convertV4MiniflareOptions({script:compiled.outputFiles[0].text,
  modules:true, compatibilityDate:compatibility.date, compatibilityFlags:compatibility.flags, cf:false, host:'127.0.0.1', port:0,
  telemetry:{enabled:false}, d1Databases:{DB:'lease-test'},
  outboundService:()=>{throw new Error('No external requests permitted')}}))
try {
  ...
  await db.prepare("INSERT INTO jobs(id,type,status,attempts) VALUES ('legacy','unsupported','processing',1)").run()
  for (const sql of readFileSync(new URL('migrations/0004_job_leases.sql',edge),'utf8').split(';').filter(value=>value.trim())) await db.prepare(sql).run()
  assert.equal((await db.prepare("SELECT status FROM jobs WHERE id='legacy'").first()).status,'pending')
  await db.prepare("DELETE FROM jobs").run()
  async function invoke(input) {
    const response = await worker.dispatchFetch('http://localhost/',{method:'POST',body:JSON.stringify(input)})
    assert.equal(response.status,200)
    return response.json()
  }
  const claim = now => invoke({operation:'claim',now})
  const finish = (job,success,now) => invoke({operation:'finish',job,success,now})
  async function insert(id,attempts=0) {await db.prepare('INSERT INTO jobs(id,type,attempts) VALUES (?, ?, ?)').bind(id,'unsupported',attempts).run()}
  ...
  assert.equal((await db.prepare("SELECT status FROM jobs WHERE id='parallel'").first()).status,'done')
  await insert('retry')
  ...
  assert.equal((await db.prepare("SELECT status FROM jobs WHERE id='exhausted'").first()).status,'failed')
  ...
  assert.equal((await db.prepare("SELECT status FROM jobs WHERE id='last-attempt'").first()).status,'processing')
  assert.equal(await claim(last.lease_expires_at),null)
  assert.equal((await db.prepare("SELECT status FROM jobs WHERE id='last-attempt'").first()).status,'failed')
  assert.equal(await finish(last,true,last.lease_expires_at+1),false)
  console.log('D1 lease verification passed: legacy recovery, concurrent claim exclusion, expiry recovery, owner fencing, retry delay and terminal crash recovery')
