...
const source = (await Promise.all(files.map((name) => readFile(resolve(assets, name), 'utf8')))).join(
  '\n'
)
...
const forbiddenRoutes = ['/deals', '/contacts', '/communications', '/compliance', '/partner', '/search']
