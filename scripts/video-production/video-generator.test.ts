...
import { loadConfig, VideoProductionAgent } from './video-generator';
...
describe('Video Production Agent', () => {
  ...
  describe('Script Parsing', () => {
    ...
    it('should parse scene structure from markdown', async () => {
      const scriptPath = path.join(
        process.cwd(),
        'docs/video-portfolio/DEMO_VIDEO_SCRIPT.md'
      );
  ...
  describe('Output Directory', () => {
    it('should create output directory structure', async () => {
      ...
      // Verify they exist
      const audioExists = await fs.access(path.join(outputDir, 'audio'))
        .then(() => true)
        .catch(() => false);
      const visualsExists = await fs.access(path.join(outputDir, 'visuals'))
        .then(() => true)
        .catch(() => false);
      const timelinesExists = await fs.access(path.join(outputDir, 'timelines'))
        .then(() => true)
        .catch(() => false);
