...
export class FloridaScraper extends BaseScraper {
  ...
  constructor() {
    super({
      state: 'FL',
      baseUrl: 'https://www.floridaucc.com/uccweb/search.aspx',
  ...
  private async performSearch(companyName: string, searchUrl: string): Promise<ScraperResult> {
    ...
    try {
      ...
      this.log('info', 'Navigating to Florida UCC search page', { companyName })
      ...
      const { filings: rawFilings, errors: parseErrors } = await page.evaluate(() => {
        ...
        const selectorPatterns = [
          ...
          '[class*="result"]',
          '[class*="filing"]',
          '[id*="GridView"] tr:not(:first-child)' // ASP.NET GridView pattern
        ]
   ...
   * Florida uses a form-based search, so we return the main search page
   */
  getManualSearchUrl(companyName: string): string {
    // Florida UCC search is form-based, return the main search URL
