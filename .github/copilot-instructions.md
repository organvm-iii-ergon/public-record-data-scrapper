...
## Git & Multi-Agent Architecture

- Follow the 4-lane stewardship constitution in `BRANCHES.md` and `.conductor/active-handoff.md`.
- Never commit directly to `main` or any `lane/*` standing branch. Cut short-lived work branches using `./scripts/agent-harness.sh claim <issue-id>`.
- Keep changes bounded to the lane's domain as codified in `docs/CONCURRENCY_BOUNDARIES.md`.
- Always run `./scripts/agent-harness.sh preflight` locally before opening PRs. PRs target their owning standing lane (`lane/heal`, `lane/verify`, etc.).
