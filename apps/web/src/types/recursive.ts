...
export interface RecursiveEnrichmentEngine {
  enrichProspect(prospectId: string, config: RecursiveEnrichmentConfig): Promise<EnrichmentTree>

  expandNode(nodeId: string, strategy: EnrichmentStrategy): Promise<EnrichmentResult>

  evaluateOpportunities(nodeId: string): Promise<EnrichmentOpportunity[]>

  learnFromOutcome(treeId: string, outcome: 'success' | 'failure', value: number): Promise<void>
...
export interface RecursiveCompetitorAnalyzer {
  analyzeCompetitor(competitorName: string, maxDepth: number): Promise<CompetitorNode>

  buildCompetitorNetwork(centerCompany: string): Promise<CompetitorNetwork>

  identifyOpportunities(network: CompetitorNetwork): Promise<OpportunityAnalysis[]>

  trackCompetitorActivity(competitorId: string): Promise<CompetitorActivity[]>

  predictMarketChanges(network: CompetitorNetwork): Promise<MarketPrediction[]>
}
...
export interface SelfLearningEngine {
  trainModel(modelType: ModelType, trainingData: unknown[]): Promise<LearningModel>

  predict(modelId: string, features: Record<string, unknown>): Promise<ModelPrediction>

  recordOutcome(predictionId: string, actualOutcome: number): Promise<void>
...
export interface DataSourceDiscoveryEngine {
  ...
  integrateSource(sourceId: string, automate: boolean): Promise<IntegrationStatus>

  monitorSources(): Promise<SourceHealthReport[]>

  suggestNewSources(basedOnNeeds: string[]): Promise<DiscoveredSource[]>
}
...
export interface RecursiveGraphEngine {
  ...
  expandGraph(graphId: string, nodeId: string, relationTypes: RelationType[]): Promise<GraphNode[]>

  query(graphId: string, query: GraphQuery): Promise<GraphQueryResult>

  findPath(graphId: string, fromNodeId: string, toNodeId: string): Promise<GraphNode[]>
