...
export function TabNavigation({ tenantDashboard = false }: { tenantDashboard?: boolean }) {
  const triggerClass = 'flex items-center gap-2 text-sm h-9 px-2 xl:px-3'
  const listClass = [
    `glass-effect hidden md:grid w-full grid-cols-6 ${tenantDashboard ? '' : 'xl:grid-cols-12'}`,
