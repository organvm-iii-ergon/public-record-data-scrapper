    ...
    if (claimActions.length > 20 && exportActions.length > 20) {
      return this.createImprovement(
        ...
        90,
        {
          ...
          risks: ['Learning curve for new shortcuts', 'Accidental bulk operations'],
          rollbackPlan: ['Disable shortcuts', 'Revert to individual operations'],
          validationCriteria: [
  ...
  private suggestUXImprovement(): ImprovementSuggestion {
    return this.createImprovement(
      ...
      85,
      {
        ...
        risks: ['Interface clutter if overdone', 'Development time investment'],
        rollbackPlan: ['Disable help features', 'Remove tooltips', 'Simplify onboarding'],
        validationCriteria: [
